...
macro_rules! map {
...

    {
        ...
        {
            let mut _map: ::crate::prelude::HashMap<_, _> = ::crate::prelude::HashMap::new();
...
macro_rules! pop_where {
    ($self:ident, $lst:ident, $($pred:tt)+) => {{
        let placeholder = ::core::mem::take(&mut $self.$lst);

        let mut remaining = ::alloc::collections::VecDeque::default();
        let mut popped = None;
        let pred = $($pred)+;
        ...
        ::core::mem::swap(&mut $self.$lst, &mut remaining);

...
macro_rules! simple_transformer {
    (
        ...
        $(#[$struct_docs])*
        #[derive(Debug, Clone)]
        pub struct $t($crate::prelude::Box<dyn $crate::core::layout::Layout>);

        impl $t {
            /// Wrap an existing layout with this transformer
            pub fn wrap(
                layout: $crate::prelude::Box<dyn $crate::core::layout::Layout>,
            ) -> $crate::prelude::Box<dyn $crate::core::layout::Layout> {
                $crate::prelude::Box::new(Self(layout))
            }
        }

        impl $crate::core::layout::LayoutTransformer for $t {
            fn transformed_name(&self) -> $crate::prelude::String {
                format!("{}<{}>", $prefix, self.0.name())
            }

            fn inner_mut(&mut self) -> &mut $crate::prelude::Box<dyn $crate::core::layout::Layout> {
                &mut self.0
            }
            ...
            fn transform_positions(
                ...
                positions: $crate::prelude::Vec<($crate::core::conn::WinId, $crate::pure::geometry::Rect)>,
            ) -> $crate::prelude::Vec<($crate::core::conn::WinId, $crate::pure::geometry::Rect)> {
                $f(r, positions)
            }
        }
    };

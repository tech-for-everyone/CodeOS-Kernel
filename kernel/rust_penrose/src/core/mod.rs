...
/// A small typed store for [State] extensions (a no_std replacement for `anymap`).
///
/// Entries are stored boxed as `Arc<RefCell<E>>` wrappers keyed by the [TypeId]
/// of that wrapper, matching the `anymap` API used by penrose's extension methods.
#[derive(Default)]
pub struct Extensions(Vec<(TypeId, Box<dyn Any>)>);

impl Extensions {
    /// Insert (or replace) an already-wrapped extension of type `E`.
    pub fn insert<E: Any + 'static>(&mut self, arc: Arc<RefCell<E>>) {
        let id = TypeId::of::<Arc<RefCell<E>>>();
        if let Some(entry) = self.0.iter_mut().find(|(t, _)| *t == id) {
            entry.1 = Box::new(arc);
        } else {
            self.0.push((id, Box::new(arc)));
        }
    }

    /// Get a shared handle to the extension of type `E` if it has been set.
    pub fn get<E: Any + 'static>(&self) -> Option<&Arc<RefCell<E>>> {
        let id = TypeId::of::<Arc<RefCell<E>>>();
        self.0
            .iter()
            .find(|(t, _)| *t == id)
            .and_then(|(_, b)| b.downcast_ref::<Arc<RefCell<E>>>())
    }

    /// Whether the storage wrapper type `Arc<RefCell<K>>` has been registered.
    pub fn contains<K: Any + 'static>(&self) -> bool {
        self.0.iter().any(|(t, _)| *t == TypeId::of::<Arc<RefCell<K>>>())
    }

    /// Remove and return the extension of type `E`, consuming its Arc wrapper.
    pub fn remove<E: Any + 'static>(&mut self) -> Option<Arc<RefCell<E>>> {
        let id = TypeId::of::<Arc<RefCell<E>>>();
        let idx = self.0.iter().position(|(t, _)| *t == id)?;
        let (_, b) = self.0.remove(idx);
        crate::any::downcast_box::<Arc<RefCell<E>>>(b).ok().map(|b| *b)
    }
}
    ...
    C: Conn,
{
    ...
    pub fn remove_extension<E: Any>(&mut self) -> Option<E> {
        let arc: Arc<RefCell<E>> = self.extensions.remove::<E>()?;
    ...
    C: Conn,
{
    ...
    /// Process a single event from the connection and flush any resulting X
    /// requests. This is the cooperative, non-blocking equivalent of [run] for
    /// use from an embedded event loop (HyperDE). Returns whether processing
    /// should continue (i.e. the WM is still running).
    pub fn process_event(&mut self, event: C::Event) -> Result<()> {
    ...
    /// Access to the underlying connection for embedded event-loop integration.
    pub fn conn_mut(&mut self) -> &mut C {
        &mut self.conn
    }

    /// Re-grab the configured key / mouse bindings on the underlying connection.
    pub fn regrab(&mut self) -> Result<()> {
        let key_codes: Vec<_> = self.key_bindings.keys().copied().collect();
        let mouse_states: Vec<_> = self.mouse_bindings.keys().cloned().collect();
        self.conn.grab(&key_codes, &mouse_states)
    }
